...
def get_data_loaders(
    ...
    data_transforms = {
        "train": transforms.Compose([
            ...
            transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1),  # Add color augmentation
            transforms.ToTensor(),
...
# Add backward compatibility for Python 2 style .next() method
import torch.utils.data

# Monkey patch the DataLoader iterator to add .next() method (legacy tests expect it)
try:
    from torch.utils.data.dataloader import _SingleProcessDataLoaderIter as _SPIter
    if not hasattr(_SPIter, "next"):
        _SPIter.next = _SPIter.__next__  # type: ignore[attr-defined]
    try:
        from torch.utils.data.dataloader import _MultiProcessingDataLoaderIter as _MPIter
        if not hasattr(_MPIter, "next"):
            _MPIter.next = _MPIter.__next__  # type: ignore[attr-defined]
    except Exception:
        pass
except Exception:
    # If torch internal API changes, silently ignore; tests may then use next() builtin.
    pass
