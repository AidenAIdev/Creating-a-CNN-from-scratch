...
try:
    ...
    # livelossplot is an optional dependency used only for interactive tracking.
    PlotLosses = None
    MatplotlibPlot = None
...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    if interactive_tracking and PlotLosses is not None and MatplotlibPlot is not None:
        if after_subplot is not None:
            liveloss = PlotLosses(outputs=[MatplotlibPlot(after_subplot=after_subplot)])
        else:
            liveloss = PlotLosses(outputs=[MatplotlibPlot()])
    else:
        liveloss = None
        if interactive_tracking and (PlotLosses is None or MatplotlibPlot is None):
            print("Warning: livelossplot not available, interactive tracking disabled")

    # Enable tracking only if requested and the liveloss object was created
    tracking_enabled = bool(interactive_tracking and liveloss is not None)
